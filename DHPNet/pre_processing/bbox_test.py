...
DATASET_CFGS = {
    ...
    "shanghaitech": {"confidence_threshold": 0.8, "min_area": 8 * 8, "cover_threshold": 0.65, "binary_threshold": 15,
                     "gauss_mask_size": 5, 'contour_min_area': 40 * 40},
    "street":{"confidence_threshold": 0.8, "min_area": 20 * 20, "cover_threshold": 0.6, "binary_threshold": 18,
               "gauss_mask_size": 5, 'contour_min_area': 40 * 40}
}
...
def extract(dataset_root, dataset_name, train):
    ...
    for idx in tqdm(range(len(dataset)), total=len(dataset)):
        ...
        all_bboxes_obj = np.array(all_bboxes, dtype=object)
        all_classes.append(cur_classes)
        all_classes_obj = np.array(all_classes, dtype=object)
    if train:
        np.save(os.path.join(os.path.join(dataset_root, dataset_name),
                             '%s_bboxes_train.npy' % dataset_name), all_bboxes_obj)
        np.save(os.path.join(os.path.join(dataset_root, dataset_name),
                             '%s_bboxes_train_classes.npy' % dataset_name), all_classes_obj)
    else:
        np.save(os.path.join(os.path.join(dataset_root, dataset_name),
                             '%s_bboxes_test.npy' % dataset_name), all_bboxes_obj)
        np.save(os.path.join(os.path.join(dataset_root, dataset_name),
                             '%s_bboxes_test_classes.npy' % dataset_name), all_classes_obj)
    print('bboxes saved!')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_name", type=str, default="street", help='dataset name')
    parser.add_argument("--train", action='store_true', help='train or test data')
    args = parser.parse_args()
    root = '/home/xinkai/data/'
